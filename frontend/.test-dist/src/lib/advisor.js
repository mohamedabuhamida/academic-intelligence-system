...
exports.getMaxAllowedCredits = getMaxAllowedCredits;
exports.computeCgpaFromCompletedCourses = computeCgpaFromCompletedCourses;
exports.buildAdvisorInsights = buildAdvisorInsights;
function getCourseDetails(courseRelation) {
    if (!courseRelation)
        return null;
    return Array.isArray(courseRelation) ? courseRelation[0] ?? null : courseRelation;
}
function getRiskCourseDetails(courseRelation) {
    if (!courseRelation)
        return null;
    return Array.isArray(courseRelation) ? courseRelation[0] ?? null : courseRelation;
}
function getCreditHours(courseRelation) {
    return Number(getCourseDetails(courseRelation)?.credit_hours ?? 0);
}
function formatCourseLabel(courseRelation) {
...
function getMaxAllowedCredits(cgpa) {
    if (cgpa < 2)
        return 12;
    if (cgpa < 3)
        return 18;
...
function computeCgpaFromCompletedCourses(studentCourses) {
...
function buildAdvisorInsights({ fullName, cgpa, totalRequiredHours, studentCourses, riskItems, latestPlan, }) {
    const completedCourses = studentCourses.filter((course) => course.status === "completed");
    ...
    const completedCredits = completedCourses.reduce((sum, course) => sum + getCreditHours(course.courses), 0);
    const activeCredits = activeCourses.reduce((sum, course) => sum + getCreditHours(course.courses), 0);
    const plannedCredits = plannedCourses.reduce((sum, course) => sum + getCreditHours(course.courses), 0);
    const upcomingCredits = activeCredits + plannedCredits;
    const resolvedCgpa = cgpa && Number(cgpa) > 0 ? Number(cgpa) : computeCgpaFromCompletedCourses(studentCourses);
