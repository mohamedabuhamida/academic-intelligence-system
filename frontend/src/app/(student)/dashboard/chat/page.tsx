...
export default function ChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    { 
      id: 1, 
      type: 'ai', 
      content: "مرحباً! أنا مساعدك الدراسي الذكي. كيف يمكنني مساعدتك في دراستك اليوم؟",
    ...
    try {
      ...
      if (data.status === 'success') {
        const aiMessage: Message = {
          ...
          content: data.answer,
          timestamp: new Date().toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' }),
          sources: ['مصدر: نظام الدراسة بالجامعة', 'دليل الطالب الأكاديمي'],
        };
      ...
      if (part.match(/^\d+\./)) {
        return (
          <div key={index} className="flex items-start gap-2 mr-4 mt-2">
            ...
            <span className="text-[#102C57]/80">{part.replace(/^\d+\.\s+/, '')}</span>
          </div>
        );
      }
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <h1 className="text-3xl font-bold text-[#102C57] mb-2">المساعد الدراسي الذكي</h1>
          <p className="text-[#102C57]/60">اسأل عن أي شيء يتعلق بدراستك، المقررات، أو الخطط الدراسية</p>
        </div>
        <motion.button
      ...
      >
        {messages.map((message, index) => (
          ...
          >
            <div className={`flex gap-3 max-w-3xl ${message.type === 'user' ? 'flex-row' : 'flex-row-reverse'}`}>
              {/* Avatar */}
              ...
              <div className="space-y-2">
                <div className={`rounded-2xl p-4 ${
                  ...
                  <div className="text-sm leading-relaxed whitespace-pre-line" style={{ direction: 'rtl' }}>
                    {message.type === 'ai' ? formatArabicText(message.content) : message.content}
                  </div>
                  
                  {/* Sources */}
                  {message.sources && (
                    <div className="mt-3 pt-3 border-t border-[#DAC0A3]/20">
                      <p className="text-xs font-medium text-[#102C57]/60 mb-2">المصادر:</p>
                ...
                {/* Message Footer */}
                <div className={`flex items-center gap-2 px-2 ${message.type === 'user' ? 'justify-start' : 'justify-end'}`}>
      ...
      >
        <div className="bg-white rounded-2xl border border-[#DAC0A3]/20 shadow-lg">
          ...
          <div className="px-4 py-3 border-b border-[#DAC0A3]/10 flex items-center gap-2 overflow-x-auto">
            <span className="text-xs font-medium text-[#102C57]/60">إجراءات سريعة:</span>
            {['متطلبات التخرج', 'المقررات', 'الخطة الدراسية', 'المعدل'].map((action) => (
              <motion.button
                key={action}
